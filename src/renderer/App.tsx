...
export default function App() {
  ...
  async function loadFeed() {
    // Use IPC to get feed through main process (which has the auth session)
  ...
  async function dismissItem(id: string) {
    // Use IPC to dismiss item through main process (which has the auth session)
  ...
  async function handleSync() {
    ...
    try {
      ...
      // Reload feed and events after sync
  ...
  if (loading) {
    return (
      <div className="h-screen flex items-center justify-center bg-gradient-oracle">
  ...
  if (!user) {
    return (
      <div className="h-screen bg-gradient-oracle flex flex-col items-center justify-center p-8">
        <div className="relative z-10 text-center animate-fade-in">
          {/* Logo with orb effect */}
          <div className="orb w-20 h-20 mx-auto mb-8 rounded-full flex items-center justify-center glow-amber"
            style={{ background: 'linear-gradient(135deg, var(--amber) 0%, var(--amber-light) 100%)' }}
          >
            <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="var(--void)" strokeWidth="1.5">
          ...
          {/* Title */}
          <h1 className="font-display text-4xl mb-3 gradient-text">
            Clairvoyant
          </h1>
          <p className="font-body text-sm mb-10" style={{ color: 'var(--cream-muted)' }}>
  ...
  return (
    <div className="h-screen overflow-auto" style={{ background: 'var(--void)' }}>
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2.5">
            <div
              ...
              style={{ background: 'linear-gradient(135deg, var(--amber) 0%, var(--amber-light) 100%)' }}
            >
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="var(--void)" strokeWidth="2">
                <circle cx="12" cy="12" r="10" />
                <path d="M12 6v6l4 2" strokeLinecap="round" />
              </svg>
            </div>
      ...
      {/* API Key Warning */}
      {!hasApiKey && (
        <div className="mx-4 mt-3 badge animate-fade-in" style={{ display: 'block', padding: '12px 16px' }}>
          <p className="text-sm font-body" style={{ color: 'var(--amber)' }}>
            Add your Anthropic API key in{' '}
            <button
              onClick={() => setShowSettings(true)}
              className="underline font-medium link-amber"
            >
              Settings
            </button>{' '}
